...
export async function seal(plaintext:string|BufferSource,recipient:HybridRecipient,aad?:string):Promise<HybridEnvelope>{
  ...
  const keyBytes=await hkdf(shared as BufferSource,new Uint8Array(32) as BufferSource,utf8("allascode/secure-communication/v1") as BufferSource,32);
  const aes=await cryptoApi.subtle.importKey("raw",keyBytes as BufferSource,"AES-GCM",false,["encrypt"]);
  const iv=randomBytes(12);
  const ciphertext=new Uint8Array(await cryptoApi.subtle.encrypt({name:"AES-GCM",iv:iv as BufferSource,additionalData:aad?utf8(aad) as BufferSource:undefined,tagLength:128},aes,typeof plaintext==="string"?utf8(plaintext) as BufferSource:plaintext));
...
export async function open(envelope:HybridEnvelope,privateKey:CryptoKey):Promise<Uint8Array>{
  ...
  const keyBytes=await hkdf(shared as BufferSource,new Uint8Array(32) as BufferSource,utf8("allascode/secure-communication/v1") as BufferSource,32);
  const aes=await cryptoApi.subtle.importKey("raw",keyBytes as BufferSource,"AES-GCM",false,["decrypt"]);
  return new Uint8Array(await cryptoApi.subtle.decrypt({name:"AES-GCM",iv:base64UrlToBytes(envelope.iv) as BufferSource,additionalData:envelope.aad?utf8(envelope.aad) as BufferSource:undefined,tagLength:128},aes,base64UrlToBytes(envelope.ciphertext) as BufferSource));
}
